...
async def get_cap(settings, remaining_seconds, files, query, total_results, search):
    ...
    if settings["imdb"]:
        ...
        if IMDB_CAP:
            ...
            for file in files:
                cap += f"<b>‼️ <a href='https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}'>{get_size(file.file_size)}➛ ʀʙ {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}\n\n</a></b>"
        else:
