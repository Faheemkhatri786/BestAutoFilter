...
async def give_filter(client, message):
    if message.chat.id != SUPPORT_CHAT_ID:
        ...
        if total_results == 0:
            ...
            return await message.reply_text(f"<b>ʜᴇʏ {message.from_user.mention}, {str(total_results)} ʀᴇsᴜʟᴛs ᴀʀᴇ ғᴏᴜɴᴅ ɪɴ ᴍʏ ᴅᴀᴛᴀʙᴀsᴇ ғᴏʀ ʏᴏᴜʀ ϙᴜᴇʀʏ {search}. \n\nᴛʜɪs ɪs ᴀ sᴜᴘᴘᴏʀᴛ ɢʀᴏᴜᴘ sᴏ ʏᴏᴜ ᴄᴀɴ'ᴛ ɢᴇᴛ ғɪʟᴇs ғʀᴏᴍ ʜᴇʀᴇ...\n\nᴊᴏɪɴ ᴀɴᴅ sᴇᴀʀᴄʜ ʜᴇʀᴇ - https://t.me/MovieZTonight</b>")
...
async def next_page(bot, query):
    ...
    try:
        if settings['max_btn']:
            ...
            if n_offset == 0:
                btn.append(
                    [InlineKeyboardButton("ᐊ", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages")]
                )
            elif off_set is None:
                btn.append([InlineKeyboardButton("ᴘᴀɢᴇ", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages"), InlineKeyboardButton("ᐅ", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
                        InlineKeyboardButton("ᐊ", callback_data=f"next_{req}_{key}_{off_set}"),
                        InlineKeyboardButton(f"{math.ceil(int(offset)/10)+1} / {math.ceil(total/10)}", callback_data="pages"),
                        InlineKeyboardButton("ᐅ", callback_data=f"next_{req}_{key}_{n_offset}")
                    ],
                )
        else:
            ...
            if n_offset == 0:
                btn.append(
                    [InlineKeyboardButton("ᐊ", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total/int(MAX_B_TN))}", callback_data="pages")]
                )
            elif off_set is None:
                btn.append([InlineKeyboardButton("ᴘᴀɢᴇ", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/int(MAX_B_TN))+1} / {math.ceil(total/int(MAX_B_TN))}", callback_data="pages"), InlineKeyboardButton("ᐅ", callback_data=f"next_{req}_{key}_{n_offset}")])
            else:
                btn.append(
                    [
                        InlineKeyboardButton("ᐊ", callback_data=f"next_{req}_{key}_{off_set}"),
...
async def filter_languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    if not files:
        await query.answer("🚫 𝗡𝗼 𝗙𝗶𝗹𝗲𝘀 𝗙𝗼𝗿 𝗧𝗵𝗶𝘀 𝗟𝗮𝗻𝗴𝘂𝗮𝗴𝗲 𝗪𝗲𝗿𝗲 𝗙𝗼𝘂𝗻𝗱 🚫", show_alert=1)
        return
    temp.GETALL[key] = files
    settings = await get_settings(message.chat.id)
    pre = 'filep' if settings['file_secure'] else 'file'
    if settings["button"]:
        ...
        btn.insert(0, [
            ...
            InlineKeyboardButton("sᴇʟᴇᴄᴛ ᴀɢᴀɪɴ ↺", callback_data=f"languages#{key}")
        ])
    else:
        btn = []
        btn.insert(0, 
    ...
    if offset != "":
        try:
            if settings['max_btn']:
                btn.append(
                    [InlineKeyboardButton("ᴘᴀɢᴇ", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/10)}",callback_data="pages"), InlineKeyboardButton(text="ᐅ",callback_data=f"next_{req}_{key}_{offset}")]
                )
    
            else:
                btn.append(
                    [InlineKeyboardButton("ᴘᴀɢᴇ", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/int(MAX_B_TN))}",callback_data="pages"), InlineKeyboardButton(text="ᐅ",callback_data=f"next_{req}_{key}_{offset}")]
                )
        except KeyError:
            await save_group_settings(query.message.chat.id, 'max_btn', True)
            btn.append(
...
async def seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    btn.insert(
        ...
        [
            InlineKeyboardButton(
                text="⇊ sᴇʟᴇᴄᴛ ᴛʜᴇ sᴇᴀsᴏɴ ⇊", callback_data="ident"
            )
        ],
    )
    req = query.from_user.id
...
async def filter_seasons_cb_handler(client: Client, query: CallbackQuery):
    ...
    if not files:
        await query.answer("🚫 𝗡𝗼 𝗙𝗶𝗹𝗲𝘀 𝗙𝗼𝗿 𝗧𝗵𝗶𝘀 𝗦𝗲𝗮𝘀𝗼𝗻 𝗪𝗲𝗿𝗲 𝗙𝗼𝘂𝗻𝗱 🚫", show_alert=1)
        return
    temp.GETALL[key] = files
    settings = await get_settings(message.chat.id)
    pre = 'filep' if settings['file_secure'] else 'file'
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if settings is not None:
            buttons = [
                [
                    InlineKeyboardButton(
                        'ʀᴇsᴜʟᴛ ᴘᴀɢᴇ : ʙᴜᴛᴛᴏɴ' if settings["button"] else 'ʀᴇsᴜʟᴛ ᴘᴀɢᴇ : ᴛᴇxᴛ',
                        callback_data=f'setgs#button#{settings["button"]}#{grp_id}',
                    )
                ],
                [
                    InlineKeyboardButton(
                        'ғɪʟᴇ sᴇɴᴅ ᴍᴏᴅᴇ : ᴍᴀɴᴜᴀʟ sᴛᴀʀᴛ' if settings["botpm"] else 'ғɪʟᴇ sᴇɴᴅ ᴍᴏᴅᴇ : ᴀᴜᴛᴏ sᴇɴᴅ',
                        callback_data=f'setgs#botpm#{settings["botpm"]}#{grp_id}',
                    )
                ],
                [
                    InlineKeyboardButton(
                        'ᴘʀᴏᴛᴇᴄᴛ ᴄᴏɴᴛᴇɴᴛ : ᴏɴ ✔' if settings["file_secure"] else 'ᴘʀᴏᴛᴇᴄᴛ ᴄᴏɴᴛᴇɴᴛ : ᴏғғ ✘',
                        callback_data=f'setgs#file_secure#{settings["file_secure"]}#{grp_id}',
                    )
                ],
                [
                    InlineKeyboardButton(
                        'ɪᴍᴅʙ ᴛᴇᴍᴘ : ᴏɴ ✔' if settings["imdb"] else 'ɪᴍᴅʙ : ᴏғғ ✘',
                        callback_data=f'setgs#imdb#{settings["imdb"]}#{grp_id}',
                    )
                ],
                [
                    InlineKeyboardButton(
                        'sᴘᴇʟʟ ᴄʜᴇᴄᴋ : ᴏɴ ✔' if settings["spell_check"] else 'sᴘᴇʟʟ ᴄʜᴇᴄᴋ : ᴏғғ ✘',
                        callback_data=f'setgs#spell_check#{settings["spell_check"]}#{grp_id}',
                    )
                ],
                [
                    InlineKeyboardButton(
                        'ᴡᴇʟᴄᴏᴍᴇ ᴍsɢ : ᴏɴ ✔' if settings["welcome"] else 'ᴡᴇʟᴄᴏᴍᴇ ᴍsɢ : ᴏғғ ✘',
                        callback_data=f'setgs#welcome#{settings["welcome"]}#{grp_id}',
                    )
                ],
                [
                    InlineKeyboardButton(
                        'ᴀᴜᴛᴏ-ᴅᴇʟᴇᴛᴇ : 10 ᴍɪɴᴛ.' if settings["auto_delete"] else 'ᴀᴜᴛᴏ-ᴅᴇʟᴇᴛᴇ : ᴏғғ ✘',
                        callback_data=f'setgs#auto_delete#{settings["auto_delete"]}#{grp_id}',
                    )
                ],
                [
                    InlineKeyboardButton(
                        'ᴀᴜᴛᴏ ғɪʟᴛᴇʀ : ᴏɴ ✔' if settings["auto_ffilter"] else 'ᴀᴜᴛᴏ ғɪʟᴛᴇʀ : ᴏғғ ✘',
                        callback_data=f'setgs#auto_ffilter#{settings["auto_ffilter"]}#{grp_id}',
                    )
                ],
                [
                    InlineKeyboardButton(
                        'ᴍᴀx ʙᴜᴛᴛᴏɴs : 10' if settings["max_btn"] else f'ᴍᴀx ʙᴜᴛᴛᴏɴs : {MAX_B_TN}',
                        callback_data=f'setgs#max_btn#{settings["max_btn"]}#{grp_id}',
                    )
                ],
                [
                    InlineKeyboardButton(
                        'sʜᴏʀᴛʟɪɴᴋ : ᴏɴ ✔' if settings["is_shortlink"] else 'sʜᴏʀᴛʟɪɴᴋ : ᴏғғ ✘',
    ...
    # don't change anything without contacting me @creatorrio
        ...
        buttons = [[
                    ...
                    InlineKeyboardButton('💥 sᴜᴘᴘᴏʀᴛ', callback_data='support_module')
                ],[
                    InlineKeyboardButton('💠 ʜᴇʟᴘ 💠', callback_data='help'),
                    InlineKeyboardButton('♻️ ᴀʙᴏᴜᴛ ♻️', callback_data='about')
                ],[
                    InlineKeyboardButton('📌 ғᴀʜᴇᴇᴍ ᴜɴɪᴠᴇʀsᴇ™ 📌', callback_data='redirect_to_faheem')
                  ]]
        reply_markup = InlineKeyboardMarkup(buttons)        
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
            InputMediaPhoto("https://telegra.ph/file/80f311edcc33f2064986f.mp4")
        )
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
        ...
        btn = [[
            ...
            InlineKeyboardButton("☎️ Cᴏɴᴛᴀᴄᴛ", url="telegram.me/Faheem21025492006")
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
    ...
    elif query.data == "support_module":
        btn = [[
            InlineKeyboardButton("• ʀǫ-ɢʀᴏᴜᴘ •", url="t.me/movieriqvestatoz"),
            InlineKeyboardButton("• ᴍᴏᴠɪᴇs ᴄʜᴀɴɴᴇʟ •", url="t.me/faheemkhatri7861")
        ], [
            InlineKeyboardButton("• ʙᴏᴛ ᴜᴘᴅᴀᴛᴇꜱ •", url="t.me/faheemkhatri7861"),
            InlineKeyboardButton("• ᴀᴅᴍɪɴ •", url="t.me/Faheem21025492006")
        ], [
        ...
        buttons = [[
            InlineKeyboardButton('🍷 ᴍᴏʀᴇ ғᴇᴀᴛᴜʀᴇꜱ 🍷', callback_data='more_features')
        ], [
            ...
            InlineKeyboardButton('• ʀᴏᴜʟs •', callback_data='rouls')
        ], [
            InlineKeyboardButton('⇋ ʙᴀᴄᴋ ᴛᴏ ʜᴏᴍᴇ ⇌', callback_data='start')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
    ...
    elif query.data == "more_features":
        buttons = [[
            InlineKeyboardButton('✨ ᴛᴇʟᴇɢʀᴀᴘʜ ʟɪɴᴋ ♻️', callback_data='tgraph')
        ], [
            InlineKeyboardButton('🗣️ ᴛᴇxᴛ ᴛᴏ ꜱᴘᴇᴇᴄʜ 🎁', callback_data='txt_to_spch')
        ], [
            InlineKeyboardButton('⚡ ғᴏɴᴛ ɢᴇɴᴇʀᴀᴛᴏʀ 📝', callback_data='fnt_gen')
        ], [
            InlineKeyboardButton('💫 ᴄᴀʀʙᴏɴ ᴛᴇxᴛ 🎀', callback_data='carbon_txt')
        ], [
            InlineKeyboardButton('🎯 sᴛɪᴄᴋᴇʀ-ɪᴅ ⚕️', callback_data='sticker_id')
        ], [
            InlineKeyboardButton('⚙️ ᴄᴏᴜɴᴛʀʏ ɪɴғᴏ 🌐', callback_data='cntry_info')
        ], [
        ...
        await asyncio.sleep(0.2)
        await query.message.edit_text(
            text=script.MORE_TXT.format(query.from_user.mention),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
        ...
        buttons = [[
            InlineKeyboardButton('⇇ ʙᴀᴄᴋ ⇉', callback_data='more_features')
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.TXT_TO_SPEECH_TXT,
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
        
    elif query.data == "fnt_gen":
        buttons = [[
            InlineKeyboardButton('⇇ ʙᴀᴄᴋ ⇉', callback_data='more_features')
        ]]
        await client.edit_message_media(
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.FNT_GEN_TXT,
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
        
    elif query.data == "carbon_txt":
        buttons = [[
            InlineKeyboardButton('⇇ ʙᴀᴄᴋ ⇉', callback_data='more_features')
        ]]
        await client.edit_message_media(
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.CRBN_TXT,
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
        
    elif query.data == "sticker_id":
        buttons = [[
            InlineKeyboardButton('‹‹‹ Bᴀᴄᴋ', callback_data='more_features')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
    ...
    elif query.data == "cntry_info":
        buttons = [[
            InlineKeyboardButton('‹‹‹ Bᴀᴄᴋ', callback_data='more_features')
        ]]
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.CONTRY_TXT,
            disable_web_page_preview=True,
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
    ...
    elif query.data == "rouls":
        buttons = [[
            InlineKeyboardButton('⇇ ʙᴀᴄᴋ', callback_data='help'),
            InlineKeyboardButton('ꜱᴛɪʟʟ ᴅᴏᴜʙᴛꜱ ?', url="t.me/movieriqvestatoz")
        ]]
        await client.edit_message_media(
            query.message.chat.id, 
            query.message.id, 
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🛰️ ʀᴇɴᴅᴇʀɪɴɢ ɪɴғᴏ ☁️', callback_data='rendering_info')
        ],[
        ...
        await query.answer("ғᴇᴛᴄʜɪɴɢ ᴍᴏɴɢᴏDB ꜱᴛᴀᴛᴜꜱ...")
        buttons = [[
            InlineKeyboardButton('⟲ Rᴇғʀᴇsʜ', callback_data='rfrsh'),
            InlineKeyboardButton('⇇ ʙᴀᴄᴋ', callback_data='about')
        ]]
        ...
        await client.answer_callback_query(
            query.id,
            text="⚡ ʟɪᴠᴇ ꜱʏꜱᴛᴇᴍ ꜱᴛᴀᴛᴜꜱ ⚡\n\n❂ ʀᴀᴍ ●●●●●●◌◌◌◌\n✇ ᴄᴘᴜ ●●●●●●◌◌◌◌\n✪ ᴅᴀᴛᴀ ᴛʀᴀғɪᴄꜱ ●●●◌◌◌◌◌◌◌🛰️\n\nᴠ𝟷.𝟹.𝟶 [ꜱᴛᴀʙʟᴇ] \n© ᴛᴇᴀᴍ ʀᴇɴɪᴇʀʙ 𝟸𝟶𝟸𝟹-𝟸𝟺",
            show_alert=True
        )
        
    elif query.data == "network_module":
        buttons = [[
            InlineKeyboardButton('⇇ ʙᴀᴄᴋ ⇉', callback_data='start')
        ]]
        await client.edit_message_media(
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.NETWORK_TXT,
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
        
    elif query.data == "redirect_to_faheem":       
        # Redirect the user to the specified URL
        await query.answer()
        await query.edit_message_text("♻️ ᴡᴀɪᴛ ᴘʟᴇᴀsᴇ... ❣️")
        await query.message.reply_text("https://t.me/Faheem21025492006")
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            search = message.text
# Select a random sticker from UPLOAD_STICKERS
            random_sticker = random.choice(UPLOAD_STICKERS)
# Create the inline keyboard button
            btn = [[
                    InlineKeyboardButton(f"sᴇᴀʀᴄʜɪɴɢ 🔍 ғᴏʀ {search}", callback_data='searchingfor')
                    ]]
# Send the random sticker with the inline keyboard markup
            m=await message.reply_sticker(random_sticker, reply_markup=InlineKeyboardMarkup(btn))
            search = search.lower()
            find = search.split(" ")
            search = ""
        ...
        if re.search(pattern, name, flags=re.IGNORECASE):
            ...
            if btn is not None:
                try:
                    if fileid == "None":
                        if btn == "[]":
                            renierb = await client.send_message(
                                group_id, 
                                reply_text, 
                                disable_web_page_preview=True,
                        ...
                        renierb = await client.send_cached_media(
                            group_id,
                            fileid,
                        ...
                        button = eval(btn)
                        renierb = await message.reply_cached_media(
                            fileid,
                            caption=reply_text or "",
                            reply_markup=InlineKeyboardMarkup(button),
                            reply_to_message_id=reply_id
