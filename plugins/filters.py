...
async def addfilter(client, message):
    ...
    if (len(extracted) >= 2) and not message.reply_to_message:
        ...
        fileid = None
        if not reply_text:
            await message.reply_text("ʏᴏᴜ ᴄᴀɴɴᴏᴛ ʜᴀᴠᴇ ʙᴜᴛᴛᴏɴ ᴀʟᴏɴᴇ, ɢɪᴠᴇ sᴏᴍᴇ ᴄᴏɴᴛᴇɴᴛ ᴛᴏ ɢᴏ ᴡɪᴛʜ ɪᴛ!", quote=True)
            return
...
async def get_all(client, message):
    ...
    if count:
        filterlist = f"ᴛᴏᴛᴀʟ ɴᴏ. ᴏғ ғɪʟᴛᴇʀs ɪɴ **{title}** : {count}\n\n"
...
async def deletefilter(client, message):
    ...
    try:
        ...
        await message.reply_text(
            "<i>ᴍᴇɴᴛɪᴏɴ ᴛʜᴇ ғɪʟᴛᴇʀ ɴᴀᴍᴇ ʏᴏᴜ ᴡᴀɴɴᴀ ᴅᴇʟᴇᴛᴇ!</i>\n\n"
            "<code>/del filtername</code>\n\n"
            "ᴜsᴇ /viewfilters ᴛᴏ ᴠɪᴇᴡ ᴀʟʟ ᴀᴠᴀɪʟᴀʙʟᴇ ғɪʟᴛᴇʀs",
            quote=True
        )
        return
    ...
    if (st.status == enums.ChatMemberStatus.OWNER) or (str(userid) in ADMINS):
        await message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton(text="ʏᴜᴘs ✅",callback_data="delallconfirm")],
                [InlineKeyboardButton(text="ɴᴏᴘᴇ ❌",callback_data="delallcancel")]
            ]),
            quote=True
        )
